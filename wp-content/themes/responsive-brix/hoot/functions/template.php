...
function hoot_get_content_template() {
	...
	$templates = apply_filters( 'hoot_content_template_hierarchy', $templates );

	/* Apply filters and return the found content template. */
	include( apply_filters( 'hoot_content_template', locate_template( $templates, false, false ) ) );
}
 ...
 * @return void
 */
function hoot_get_sidebar( $name = null ) {
 ...
 * hoot-theme sub-folder or the hoot framework folder.
 *
 * @since 1.1.0
 * @access public
 * @param string  $name
...
function hoot_locate_shortcode( $name = '' ) {
	...
	if ( '' !== $name ) {
		...
		$base = str_replace( trailingslashit( THEME_DIR ), '', trailingslashit( HOOT_DIR ) );
